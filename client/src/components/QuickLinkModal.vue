  ...
  <div class="modal-backdrop" @click.self="$emit('cancel')">
    <div class="modal">
      ...
      <form class="modal-body" @submit.prevent="save">
        <div class="field">
          <label>Label</label>
          <input v-model="form.label" type="text" placeholder="e.g. Jira Board" required />
        </div>
        <div class="field">
          <label>URL</label>
          <input v-model="form.url" type="text" placeholder="https://..." required />
      ...
      <div class="modal-footer">
        ...
        <div class="confirm-delete" v-if="link && confirmDelete">
...
<script setup>
import { ref } from 'vue';
import { createQuickLink, updateQuickLink, deleteQuickLink } from '../api.js';
...
async function save() {
  ...
  if (!form.value.url.trim().startsWith('http://') && !form.value.url.trim().startsWith('https://')) {
  ...
  try {
    let result;
    if (props.link) {
      result = await updateQuickLink(props.link.id, { label: form.value.label, url: form.value.url });
    } else {
      result = await createQuickLink({ label: form.value.label, url: form.value.url });
    }
    emit('save', result);
  } catch (e) {
...
async function doDelete() {
  await deleteQuickLink(props.link.id);
  emit('save', null);
}
</script>
